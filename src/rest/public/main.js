...
$(function () {
    ...
    jQuery("#courseexplore").submit(function (e) {
       ...
       if(filterresult.length===1&&filterresult[0]==="courses_dept")
       {
           ...
           if(keyarray.length>0)
           {  query=JSON.stringify({
                   "GET": ["courses_dept", "courses_id","AvgPass","AvgFailing","AvgGrade","Avgsize"],
                   "WHERE":departmentquery,
                   "GROUP": ["courses_dept","courses_id" ],
                   "APPLY": [ {"AvgPass": {"AVG": "courses_pass"}},
                       {"AvgFailing": {"AVG":"courses_fail"}},{"AvgGrade": {"AVG":"courses_avg" }},{"Avgsize": {"AVG": "courses_size"}}],
                   "ORDER": orderquery,
                   "AS":"TABLE"})}
                   else
           {   query=JSON.stringify({
   ...
   try {
    ...
    jQuery("#schedulingformbykey1").submit(function(e) {
        e.preventDefault();
        var roomlist = jQuery("#roomlist4").val()
        var courselist = jQuery("#courselist4").val()
    ...
    jQuery("#schedulingformbylist1").submit(function(e) {
        e.preventDefault();
        var roomlist = jQuery("#roomlist3").val()
        var courselist = jQuery("#courselist3").val()
    ...
    jQuery("#schedulingformbykey2").submit(function(e) {
        e.preventDefault();
        console.log("jump to else")
        var department2 = jQuery("#coursedept2").val().replace(/ /g, '')
        var buildingname2 = jQuery("#buildingname6").val().replace(/ /g, '')
        var buildinglocation=jQuery("#buildingname7").val()
        var realdistance=jQuery("#roomdistance2").val()
